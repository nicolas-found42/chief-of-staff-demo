...
test("tasks journey — a Debrief's Action Items arrive as proposals, not Tasks", async ({
  ...
  await expect(proposal.getByRole("link", { name: "Open source Debrief" })).toHaveAttribute(
    "href",
    `/meeting-debrief/${runId}?retained=1`,
  );
...
test("tasks journey — a restricted release keeps the saved preference pending without attestation controls", async ({
  page,
  request,
}) => {
  await page.goto("/tasks");
  await expect(page.getByRole("heading", { level: 1, name: "Tasks" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Automatically create my Tasks" })).toBeVisible();

  for (const text of [
    /sha256/i,
    /release evidence/i,
    /release-restriction/i,
    /release-evidence-not-recorded/i,
  ]) {
    await expect(page.getByText(text)).toHaveCount(0);
  }
  await expect(page.getByRole("button", { name: "Record the release evidence" })).toHaveCount(0);

  await page.getByRole("button", { name: "Automatically create my Tasks" }).click();
  const statusText = async () => (await page.getByRole("status").allTextContents()).join(" ");
  await expect.poll(statusText).toMatch(/saved|pending/i);
  await expect.poll(statusText).toMatch(/pending|unavailable|not active/i);
  await expect
    .poll(statusText)
    .toMatch(/later release[\s\S]*explicit|explicit[\s\S]*later release/i);
  await expect(page.getByRole("button", { name: "Stage all Action Items" })).toBeVisible();
  await expect(page.getByText("My own commitments now become Tasks automatically.")).toHaveCount(0);
  ...
  expect(body.automaticPromotion.effective).toBe(false);
