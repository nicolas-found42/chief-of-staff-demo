...
it("refuses a rejected corpus instead of evaluating its remaining entries", async () => {
...
])("rejects invalid selection: $error", async ({ args, error }) => {
...
it("records a valid limited subset as intentional and preserves its failed evaluation", async () => {
...
it("records all default corpus members excluded by --limit without an explicit --people list", async () => {
  const output = mkdtempSync(join(tmpdir(), "benchmark-default-limit-"));
  try {
    const config = join(output, "config.json");
    writeFileSync(config, JSON.stringify({ provider: "mock", model: "mock" }));
    const result = await runBenchmarkCli(["--config", config, "--limit", "1", "--out", output]);
    expect(result.status).toBe(1);
...
])("rejects unsafe benchmark concurrency: $args", async ({ args, error }) => {
  const result = await runBenchmarkCli(args);
  expect(result.status).toBe(1);
  expect(result.stderr).toContain(error);
});

it("retains public evidence without credentials and produces a separately reassessable artifact", async () => {
  ...
  try {
    ...
    const reassessed = await runBenchmarkCli([
      "--reassess",
...
it("--retry carries an eligible person and re-runs only the rest", async () => {
...
it("--retry with --repeats 2 records carried people once: later repeats sample everyone fresh", async () => {
...
it("--retry refuses a stamped artifact from a different run recipe", async () => {
...
it("--repeats 2 writes one report per repeat plus arm statistics", async () => {
  ...
  try {
    ...
    const again = await runBenchmarkCli(["--stats", dir]);
...
it("--compare-stats pairs two arm statistics files and warns inside the noise band", async () => {
...
it("--seed is recorded and a non-integer seed is rejected", async () => {
  ...
  try {
    ...
    const bad = await runBenchmarkCli([
      "--config",
      config,
      "--seed",
      "1.5",
