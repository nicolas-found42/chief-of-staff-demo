...
export function hasSeenDriveFile(stateFile: string, externalId: string): boolean {
  return hasSeen(stateFile, externalId);
}
/**
 * Load-modify-save guarded remember. Single writer at a time via file re-read.
 */
export function rememberSeenDriveFile(stateFile: string, externalId: string): void {
  const state = loadState(stateFile);
...
function forgetSeenDriveFile(stateFile: string, externalId: string): void {
  const state = loadState(stateFile);
  const ids = state.drive.ingestedIds;
...
export function reclaimStrandedDriveRun(args: {
  ...
  for (const summary of args.runs.list({ module: args.moduleId }).runs) {
    ...
    if (meta.externalId) forgetSeenDriveFile(args.stateFile, meta.externalId);
    reclaimed += 1;
  }
  return reclaimed;
}
