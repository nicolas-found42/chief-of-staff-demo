...
export async function runBenchmarkCli(
  ...
  const resolvePath = (p: string) => resolve(repoRoot, p);
  ...
  try {
    ...
    const corpusDir = resolvePath(arg("corpus") ?? "benchmark/person-research/people");

    if (flag("corpus-coverage")) {
      const corpus = loadCorpus(corpusDir);
      writeStdout(`corpus ${corpus.version}: ${String(corpus.people.length)} people\n`);
      for (const rejection of corpus.rejected)
        writeStdout(`REJECTED ${rejection.file}: ${rejection.reason}\n`);
      ...
      return { status: corpus.rejected.length ? 1 : 0, stdout, stderr };
    }

    const comparisonIndex = args.indexOf("--compare");
    if (comparisonIndex !== -1) {
      const rawBaseline = args[comparisonIndex + 1];
      const rawCandidate = args[comparisonIndex + 2];
      if (!rawBaseline || !rawCandidate)
        throw new Error("--compare needs a baseline report path and a candidate report path.");
      const baselinePath = resolvePath(rawBaseline);
      const candidatePath = resolvePath(rawCandidate);
      const baseline = BenchmarkReportSchema.parse(JSON.parse(readFileSync(baselinePath, "utf8")));
      const candidate = BenchmarkReportSchema.parse(
      ...
      writeStdout(`${renderComparison(comparison)}\n`);
      return {
        status: !comparison.comparable || comparison.verdict === "regressed" ? 1 : 0,
        stdout,
        stderr,
      };
    }

    const compareStatsIndex = args.indexOf("--compare-stats");
    if (compareStatsIndex !== -1) {
      const rawBaseline = args[compareStatsIndex + 1];
      const rawCandidate = args[compareStatsIndex + 2];
      if (!rawBaseline || !rawCandidate)
        throw new Error("--compare-stats needs a baseline and a candidate stats.json path.");
      const baselinePath = resolvePath(rawBaseline);
      const candidatePath = resolvePath(rawCandidate);
      const baseline = BenchmarkArmStatsSchema.parse(
        JSON.parse(readFileSync(baselinePath, "utf8")),
      );
      const candidate = BenchmarkArmStatsSchema.parse(
        JSON.parse(readFileSync(candidatePath, "utf8")),
      );
      ...
      writeStdout(`${renderStatsComparison(comparison)}\n`);
      return { status: 0, stdout, stderr };
    }

    const rawStatsDir = arg("stats");
    if (rawStatsDir !== undefined) {
      const statsDir = resolvePath(rawStatsDir);
      ...
      writeStdout(`${renderArmStats(stats)}\n`);
      return { status: 0, stdout, stderr };
    }
    ...
    const retryDir = arg("retry") ? resolvePath(arg("retry")!) : undefined;
    const reuseOnlyDir = arg("reuse-extraction")
      ? resolvePath(arg("reuse-extraction")!)
      : undefined;
   ...
   asked for a rebuild without research, which the plain flags already do. */
    ...
    const configPath = resolvePath(arg("config") ?? "workspace/config.json");
    ...
    const corpus = loadCorpus(corpusDir);
    for (const rejection of corpus.rejected)
      writeStderr(`REJECTED ${rejection.file}: ${rejection.reason}\n`);
    if (corpus.rejected.length) throw new Error("Benchmark corpus contains rejected entries.");

    const rawReassessment = arg("reassess");
    if (rawReassessment) {
      const reassessmentPath = resolvePath(rawReassessment);
      const rawEvidenceWorkspace = arg("evidence-workspace");
      if (!rawEvidenceWorkspace) throw new Error("--reassess requires --evidence-workspace.");
      const evidenceWorkspace = resolvePath(rawEvidenceWorkspace);
      if (
        [
          "people",
          "limit",
      ...
      writeStdout(`${renderReport(report, corpus.people)}\n\nWrote ${join(out, `${stem}.json`)}\n`);
      return { status: report.status === "completed" ? 0 : 1, stdout, stderr };
    }
   ...
   outage and were re-run from scratch now pay only for what is missing. */
    ...
    const outDir = resolvePath(arg("out") ?? "artifacts/person-benchmark");
   ...
   resume of identical questions is free. Local disk, never committed. */
    const judgePort = (repeat: number) =>
      cachedCompleteJson(measured(judging), {
        cacheDir: resolvePath(join(".cache", "person-benchmark", "judge")),
    ...
    for (let repeat = 1; repeat <= repeats; repeat++) {
          ...
          provenance: {
            ...
            researchSettings: {
             ...
             constants, recorded so every arm names the machinery it ran on. */
              ...
              extractionBindingPreference:
                "forced_tool_call when model-declared; default otherwise",
        ...
        if (mode === "live-discovery") {
         ...
         learns about bad routes, the way the long-running app does (#233). */
          ...
          const workers = Array.from(
            { length: Math.min(concurrency, repeatRun.length) },
            async () => {
     ...
     sampled its people, so later repeats remain meaningful. */
      ...
      writeFileSync(join(outDir, `${stemRepeat}.json`), `${JSON.stringify(validated)}\n`);
      writeFileSync(join(outDir, `${stemRepeat}.md`), `${renderReport(validated, selected)}\n`);
      writeStdout(`${renderReport(validated, selected)}\n`);
      writeStdout(
    ...
    return { status: allCompleted ? 0 : 1, stdout, stderr };
  } catch (error) {
...
if (isMain()) {
  const result = await runBenchmarkCli(process.argv.slice(2), { forwardOutput: true });
