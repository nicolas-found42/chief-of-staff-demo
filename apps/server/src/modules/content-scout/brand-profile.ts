...
import { modelBoundaryDiagnostic, parseResultShape } from "../../llm/failure.js";
  ...
  return {
    ...
    async run(ctx, input): Promise<RunOutcome> {
      ...
      await ctx.stage("propose", async () => {
        const proposedMarkdown = await deps.proposer.propose({ pages }).catch((error: unknown) => {
          const diagnostic = modelBoundaryDiagnostic(error);
          if (
            diagnostic &&
            (diagnostic.status === 401 ||
              diagnostic.upstreamCode === 401 ||
              diagnostic.upstreamCode === 403)
          ) {
            throw new StageFailure(
              "provider_key_rejected",
              `The configured ${diagnostic.provider} API key was rejected. Replace it in Settings.`,
              { eventDetail: { modelBoundary: diagnostic } },
            );
          }
          throw error;
        });
