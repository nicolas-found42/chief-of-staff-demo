...
export function IdeaEnginePage() {
  ...
  return (
    <section className="page">
      <h1>Idea Engine</h1>
      <p className="muted">
        Content Ideas extracted from transcripts, grouped by meeting, filterable by type.
      </p>
      ...
      <div className="toolbar">
        <label>
          ...
          <select
            value={filter}
            onChange={(e) => setFilter(e.target.value as IdeaContentType | "all")}
          >
        ...
        <button type="button" onClick={() => void onBackfill()} disabled={backfilling}>
      ...
      {filteredRuns.length === 0 ? (
        <p className="muted">
          No ideas yet. New transcripts after live are handled automatically; use backfill for
          history.
        </p>
      ) : (
            ...
            {run.sourceUrl && (
              <p className="muted">
                ...
                {data.spreadsheet && (
                  <>
                    {" · "}
                    <a href={data.spreadsheet.url} target="_blank" rel="noreferrer">
                      Open Sheet — {data.spreadsheet.id}
                    </a>
                  </>
                )}
              </p>
              ...
              {run.ideas.map((idea, idx) => (
                <li key={`${idea.Title}-${idx}`}>
                  ...
                  <span className="muted">
                    ...
                    —{" "}
                    <Link
                      to={`/runs/${run.runId}#${encodeURIComponent(idea.evidence.at)}`}
                      title={`Evidence quote: ${idea.evidence.quote}`}
                    >
                      evidence at {idea.evidence.at}
                    </Link>
                    : “{idea.evidence.quote}” (confidence {idea.confidence})
                  </span>
                  {data.spreadsheet && (
                    <span className="muted">
                      {" "}
                      ·{" "}
                      <a
                        href={`${data.spreadsheet.url}#row=${idx + 2}`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        Sheet row
